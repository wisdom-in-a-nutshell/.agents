...
- Reuse the shared Codex global guidance source for machine-wide behavior instead of maintaining a separate Claude policy fork.
...
- Keep machine-wide guidance in `../codex/config/global.agents.md`; `config/global.claude.md` must remain a symlink alias to that source.
