...
section "memory write — plain default success"
# Default mode is plain one-liner
BEFORE=$(wc -c < "$SCRATCH")
...
run_dobby_stdin "Second appended line" memory write --section area.builder.dobby-test-scratch --json --message "second contract test"
assert_exit "exit 0" 0 "$CAPTURED_EXIT"
