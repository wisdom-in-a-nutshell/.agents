...
def _ensure_api_env(dry_run: bool) -> None:
    ...
    if endpoint and api_key:
        print("LLM_API_ENDPOINT and LLM_API_KEY are set.", file=sys.stderr)
        return
    if dry_run:
        _warn("LLM_API_ENDPOINT or LLM_API_KEY is not set; dry-run only.")
        return
    _die("LLM_API_ENDPOINT and LLM_API_KEY must both be set. Export them before running.")
