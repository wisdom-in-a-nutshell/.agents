...
1. Read `{{body_map_path}}` for the workspace routing contract, then read the inputs manifest and work through the listed inputs: session folders in the window, journal days, `memory/now.md`, `state/shelf.json`, active project trackers, and the area manifests. Open area canon/log files only when a session, journal entry, or candidate points at them.
...
4. Across these episodes, find what per-session capture cannot see: repeated themes that deserve an area-log fact or canon update, `now.md` items whose forcing function has passed, duplicate or near-duplicate session records, commitments that never landed on the Shelf, project trackers that drifted from reality, and behavioral lessons Dobby itself should learn.
...
- `now` — update to this week's active orientation (`memory/now.md`).
- `area_log` — concrete dated fact/event for `memory/areas/<area>/log.jsonl`.
- `area_canon` — durable domain understanding for `memory/areas/<area>/canon.md`.
- `soul` — durable identity/value/boundary change for `dobby/constitution.md` or `memory/profile.md`. **Never applied — always `needs_adi`.**
...
- Never edit `dobby/constitution.md` or `memory/profile.md`.
...
Rules: every candidate cites at least one evidence path from this workspace. `action` is `applied`, `needs_adi`, or `noop`. `risk` is `low` only for append-only dated facts and tracker corrections; anything touching `now.md`, canon, or interpretation of personal material is `medium` or `high` — still applicable, but say so honestly so Adi knows where to look first.
...
- One canonical home per fact; never duplicate session prose into `now.md`.
- Respect memory clocks: constitution/profile slow (and not yours), `now.md` weekly, canon as needed.
