...
DevWorker is the planned Mac mini workflow for turning loose human intent into local Codex implementation work. Things 3 stays the capture surface, GitHub Issues becomes the durable execution queue, and Codex app-server runs the actual coding session with the target repo as `cwd`.

The first implementation should be intentionally small: one issue at a time, existing repo directory on `main`, local checks, commit/push through the repo's normal flow, and GitHub issue updates. Branches, worktrees, SQLite state, launchd, and Things automation are later layers after the core loop works.
...
flowchart TD
    ...
    C[Target repo cwd<br/>/Users/dobby/GitHub/repo]
    D[Main branch<br/>clean repo]
...
The first DevWorker is a manual CLI in `/Users/dobby/GitHub/devworker`. It takes one GitHub issue, verifies the target repo directory is clean and on `main`, starts Codex app-server with that directory as `cwd`, runs checks, and updates the issue after Codex commits through the repo's normal flow.
...
The MVP runs in the existing target repo directory under `/Users/dobby/GitHub/<repo>` on `main`.

That means it is one issue at a time per repo. The repo directory must be on `main` and clean before DevWorker starts. This keeps the first version understandable and avoids adding branch/worktree cleanup before it is needed.
...
6. DevWorker verifies the target repo directory is clean and on `main`.
7. DevWorker starts a Codex app-server thread with the target repo as `cwd`.
...
- Codex owns implementation inside the target repo `cwd`.
...
Add branches/worktrees when DevWorker needs parallel runs or when modifying the normal repo directory becomes too disruptive. Add SQLite when local crash recovery needs more than GitHub labels/comments and JSONL logs.
