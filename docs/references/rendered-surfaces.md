...
| `/Library/Application Support/ClaudeCode/managed-settings.json` (macOS) / `/etc/claude-code/managed-settings.json` (Linux) | live Claude OS-level policy file (locks plugin names against `--plugin-dir` injection by daemons such as the Claude.ai app's ccd-cli runtime) | `claude/config/managed-settings.json` | `claude/scripts/sync-managed-settings.sh --apply` (uses sudo when the existing file is root-owned) | `claude/scripts/check-claude-control-plane.sh` | No |
