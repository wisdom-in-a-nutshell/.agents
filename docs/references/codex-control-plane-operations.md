...
- `~/.codex/config.toml` uses the managed Azure OpenAI provider for the global Codex default: `model = "gpt-5.5"`, `model_provider = "azure"`, and `[model_providers.azure]` from [`global.config.toml`](/Users/dobby/.agents/codex/config/global.config.toml). Provider auth uses Azure CLI to mint short-lived Microsoft Entra tokens for the documented `https://cognitiveservices.azure.com/.default` scope; do not store those access tokens in Key Vault, `~/.secrets`, or git.
- `~/.codex/azure-key.config.toml` is the managed API-key alternative profile rendered from [`azure-key.config.toml`](/Users/dobby/.agents/codex/config/azure-key.config.toml). It uses `model_provider = "azure-key"` and reads `AZURE_OPENAI_API_KEY`; the key is generated into `~/.secrets/azure-openai/env` from the shared machine-secret mapping in `~/GitHub/scripts/sync/machine-secrets/azure-openai.env.map`.
