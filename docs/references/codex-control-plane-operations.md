...
- `~/.codex/auth.json` uses `auth_mode = "chatgpt"`. In the normal global default, `~/.codex/config.toml` leaves `model_provider` unset, so Codex uses the default OpenAI ChatGPT account provider. [`global.config.toml`](/Users/dobby/GitHub/agents/codex/config/global.config.toml) still pins `model = "gpt-5.5"`, reasoning defaults, and `service_tier = "priority"`.
...
- Keep Codex on the priority service tier by default through the top-level `service_tier = "priority"` setting. Do not add separate desktop-only service-tier keys such as `[desktop] default-service-tier = "priority"` unless Codex has a verified compact-safe desktop service-tier path.
