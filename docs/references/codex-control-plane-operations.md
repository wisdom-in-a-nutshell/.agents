...
- `~/.codex/auth.json` uses `auth_mode = "chatgpt"`. In the normal global default, `~/.codex/config.toml` leaves `model_provider` unset, so Codex uses the default OpenAI ChatGPT account provider. [`global.config.toml`](/Users/dobby/GitHub/agents/codex/config/global.config.toml) also leaves `model` and `service_tier` unset so a client's new-thread selection wins and Codex starts on its normal Standard tier; high reasoning defaults remain managed.
