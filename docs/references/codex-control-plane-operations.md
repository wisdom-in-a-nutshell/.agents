...
- Xcode runs Codex from `~/Library/Developer/Xcode/CodingAssistant/codex`, so `sync-config.sh --apply --xcode-only` renders the Xcode config/rules parity surface there, seeds Xcode's no-prompt coding-assistant permission default, and links `auth.json` / `.credentials.json` back to the normal `~/.codex` files when those sources exist. This keeps one Codex login source without copying credentials or overwriting Xcode-owned MCP/session/tool state.
