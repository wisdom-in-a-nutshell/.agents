...
- `codex/` holds canonical personal Codex control-plane inputs. The Codex sync renders terminal Codex config under `~/.codex` and the Xcode Codex parity surface under `~/Library/Developer/Xcode/CodingAssistant/codex`, including config/rules, Xcode's no-prompt coding-assistant permission preference, plus auth symlinks back to the normal `~/.codex` credential files when they exist.
