...
- `config/grok-managed-config.toml` is the canonical managed overlay for Grok Build's per-user managed config at `~/.grok/managed_config.toml`. `scripts/sync-grok.py` renders it. Keep it narrow: use it for Grok-native compatibility/config switches, not secrets or ordinary user preferences. Current policy disables Claude hook compatibility only, so Grok does not read Claude-only lifecycle hook shapes while still reusing Claude/AGENTS instructions, skills, and MCP compatibility.
...
- Do not hand-edit `~/.grok/managed_config.toml`; update `config/grok-managed-config.toml` and rerun `scripts/sync-grok.sh --apply` or the shared bootstrap. Do not put Grok auth tokens, API keys, or ordinary UI preferences in the managed config layer.
