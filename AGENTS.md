...
- `dev-servers/registry.json` is the canonical agent-preview server registry. The shared client sync renders per-repo `.claude/launch.json`, `.codex/environments/environment.toml`, and `.github/github-app.yml` from it. It is opt-in per repo: a repo gets a launch surface only if listed. Keep public Cloudflare/LaunchAgent service ports in `~/GitHub/scripts`, not this preview registry.
