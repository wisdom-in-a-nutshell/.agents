...
- A repo's identity prompt is declared once, as `model_instructions_file` in `codex/config/repo-bootstrap.json`, and reaches every client from there: Codex through `.codex/config.toml`, Copilot through a generated `.github/copilot-instructions.md` symlink, Claude Code through an `@` import line in the generated `.claude/CLAUDE.md`. Do not hand-edit those surfaces; change the registry entry and re-run the sync scripts.
