...
class CopilotSyncTests(TempDirTestCase):
    ...
    def test_disabled_repo_prunes_managed_copilot_preview_config(self) -> None:
        home = self.temp_path / "home"
        github_root = home / "GitHub"
        ...
        repo_registry = write_json(
            home / "GitHub/agents/codex/config/repo-bootstrap.json",
            {
                "defaults": {},
                "repos": [
                    {"path": "~/GitHub/codex-only", "enabled_clients": ["codex"]}
                ],
            },
        )
        dev_servers = write_json(
        ...
        self.assertFalse((repo / ".github/github-app.yml").exists())
