...
APPLY=0
LABEL="com.${USER}.claude-session-archiver"
INTERVAL_SECONDS=3600
OLDER_THAN_HOURS=24
RUN_AT_LOAD=1
CONTROL_PLANE_ROOT="${AGENTS_CONTROL_PLANE_ROOT:-${HOME}/GitHub/agents}"
SCRIPT_PATH="${CONTROL_PLANE_ROOT}/codex/scripts/archive-stale-claude-sessions.py"
PLIST_PATH="${HOME}/Library/LaunchAgents/${LABEL}.plist"
LOG_DIR="${HOME}/.local/state/codex-control-plane/log"
OUT_LOG="${LOG_DIR}/archive-stale-claude-sessions.out.log"
ERR_LOG="${LOG_DIR}/archive-stale-claude-sessions.err.log"

usage() {
  cat <<USAGE
Usage: $(basename "$0") [options]

Install/update a LaunchAgent that archives stale Claude Desktop sidebar sessions by age.
...
Options:
  ...
  --label <label>            LaunchAgent label (default: com.<user>.claude-session-archiver)
...
Examples:
  ~/GitHub/agents/codex/scripts/install-archive-stale-claude-sessions-launchagent.sh
...
printf 'Interval: %ss\n' "$INTERVAL_SECONDS"
printf 'Archive threshold: lastActivityAt older than %sh\n' "$OLDER_THAN_HOURS"
printf 'Logs:\n  %s\n  %s\n' "$OUT_LOG" "$ERR_LOG"
