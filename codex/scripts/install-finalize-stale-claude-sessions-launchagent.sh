...
APPLY=0
LABEL="com.${USER}.claude-session-finalizer"
...
SCRIPT_PATH="${CONTROL_PLANE_ROOT}/codex/scripts/finalize-stale-claude-sessions.py"
PLIST_PATH="${HOME}/Library/LaunchAgents/${LABEL}.plist"
LOG_DIR="${HOME}/.local/state/claude-control-plane/log"
OUT_LOG="${LOG_DIR}/finalize-stale-claude-sessions.out.log"
ERR_LOG="${LOG_DIR}/finalize-stale-claude-sessions.err.log"

usage() {
  cat <<USAGE
Usage: $(basename "$0") [options]

Install/update a LaunchAgent that finalizes stale Claude Code sessions through
the global finalize-claude-session command. Default mode is dry-run. Use
--apply to write and load.

Options:
  --apply                    Write and load the LaunchAgent
  --dry-run                  Print the plist only (default)
  --label <label>            LaunchAgent label (default: com.<user>.claude-session-finalizer)
  ...
  --max-sessions <n>         Sessions finalized per run (default: 10)
  --max-report <n>           Candidate detail lines kept in launchd logs (default: 25)
  --script <path>            Stale-session finalizer script path
  --run-at-load              Run once immediately when loaded (default)
  --no-run-at-load           Wait until the first interval fires
  -h, --help                 Show this help

Examples:
  ~/GitHub/agents/codex/scripts/install-finalize-stale-claude-sessions-launchagent.sh
  ~/GitHub/agents/codex/scripts/install-finalize-stale-claude-sessions-launchagent.sh --apply
USAGE
}
...
<plist version="1.0">
  <dict>
    ...
    <array>
      ...
      <string>--max-sessions</string>
      <string>$(xml_escape "$MAX_SESSIONS")</string>
...
[[ -x "$SCRIPT_PATH" ]] || die "stale-session finalizer script is not executable: $SCRIPT_PATH"
...
is_int "$MAX_SESSIONS" || die "invalid --max-sessions: $MAX_SESSIONS"
...
printf 'Interval: %ss\n' "$INTERVAL_SECONDS"
printf 'Finalize threshold: transcript idle longer than %sh\n' "$OLDER_THAN_HOURS"
printf 'Per-run cap: %s sessions\n' "$MAX_SESSIONS"
printf 'Logs:\n  %s\n  %s\n' "$OUT_LOG" "$ERR_LOG"
