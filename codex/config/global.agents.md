...
- Shared hook dispatch also supports repo-owned Python lifecycle hooks at `scripts/hooks/session_start.py`, `scripts/hooks/user_prompt_submit.py`, `scripts/hooks/pre_compact.py`, `scripts/hooks/post_compact.py`, and `scripts/hooks/session_end.py` when those events are assigned to the repo and those files exist. `PreCompact` and `PostCompact` currently render for Codex and Claude; `SessionEnd` currently renders for Claude and GitHub Copilot, not Codex.
