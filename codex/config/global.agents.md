...
- Shared hook dispatch supports repo-owned Python lifecycle hooks at `scripts/hooks/session_start.py`, `scripts/hooks/user_prompt_submit.py`, and `scripts/hooks/session_end.py` when those events are assigned to the repo in `~/.agents/hooks/registry.json` and those files exist. `SessionEnd` currently renders only for Codex-managed repos assigned in `~/.agents/hooks/registry.json`. Explicit thread finalization can also run repo-owned `scripts/hooks/finalize_codex_thread.py` through the global `finalize-codex-thread` command.
