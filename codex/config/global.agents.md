...
- Shared hook dispatch also supports repo-owned Python lifecycle hooks at `scripts/hooks/session_start.py`, `scripts/hooks/user_prompt_submit.py`, `scripts/hooks/pre_compact.py`, `scripts/hooks/post_compact.py`, and `scripts/hooks/session_end.py` when those events are assigned to the repo and those files exist. `PreCompact` currently renders for Codex and Claude. `PostCompact` remains supported by the dispatcher but is not rendered for the Dobby workspaces by default. `SessionEnd` renders for Codex, Claude, and GitHub Copilot where the runtime surfaces that event.
