...
- Shared hook dispatch also supports repo-owned `scripts/hooks/session-start.sh`, `scripts/hooks/user-prompt-submit.sh`, and Claude-only `scripts/hooks/session-end.sh` when those files exist.
