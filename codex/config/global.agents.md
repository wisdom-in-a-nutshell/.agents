...
- Shared hook dispatch supports repo-owned Python lifecycle hooks at `scripts/hooks/session_start.py` and `scripts/hooks/user_prompt_submit.py` when those events are assigned to the repo in `~/.agents/hooks/registry.json` and those files exist. There is no managed fake `SessionEnd`; explicit thread finalization can run repo-owned `scripts/hooks/finalize_codex_thread.py` through the global `finalize-codex-thread` command.
