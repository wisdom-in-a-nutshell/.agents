...
- Shared hook dispatch also supports repo-owned Python lifecycle hooks at `scripts/hooks/session_start.py`, `scripts/hooks/user_prompt_submit.py`, `scripts/hooks/pre_compact.py`, `scripts/hooks/post_compact.py`, and `scripts/hooks/session_end.py` when those events are assigned to the repo in `~/.agents/hooks/registry.json` and those files exist. `PreCompact` is supported by the dispatcher but is not enabled for Dobby workspaces by default. `PostCompact` currently renders for Dobby workspaces when enabled in `~/.agents/hooks/registry.json`. `SessionEnd` renders for Codex, Claude, and GitHub Copilot where the runtime surfaces that event.
