...
- Shared hook dispatch also supports repo-owned Python lifecycle hooks at `scripts/hooks/session_start.py`, `scripts/hooks/user_prompt_submit.py`, `scripts/hooks/pre_compact.py`, `scripts/hooks/post_compact.py`, and `scripts/hooks/session_end.py` when those events are assigned to the repo in `~/.agents/hooks/registry.json` and those files exist. `PostCompact` and `SessionEnd` currently render only for Codex-managed repos assigned in `~/.agents/hooks/registry.json`.
